...
/*AIADDED by mdpar*/
	if (AIship[AIship_find(other->id)].shape != 0) {
		Free_ship_shape((shipobj *)AIship[AIship_find(other->id)].shape);
	}
	else
		AIship[AIship_find(other->id)].id = other->id;
	AIship[AIship_find(other->id)].shape = (struct shipobj *) other->ship;
