...
/*AIADDED by mdpar*/
	if (self != NULL && id == self->id) {
		AIself.id = self->id;
		AIself.team = self->team;
		AIself.life = self->life;
		//AIself.score = self->score;	
		//printf("score: %f\n",self->score);
	}
	else if (other != NULL) {
		AIship[AIship_find(id)].life = other->life;
		AIship[AIship_find(id)].team = other->team;
		AIship[AIship_find(id)].name = other->name;
	}	
