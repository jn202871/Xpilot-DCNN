...
def save_scores_to_db(epoch, tracc, valacc):
    """Manager helper func to save scores to db"""
    conn = sqlite3.connect('/home/jnash/Research/Xpilot-DCNN/acc_data.db')
    c = conn.cursor()
    c.execute('''
    ...
    ''')
    c.execute('INSERT INTO data (epoch, tracc, valacc) VALUES (?, ?, ?)',
              (epoch, tracc, valacc))
    conn.commit()
    conn.close()
...
for i in range(10):
    ...
    for epoch in range(epochs):
        ...
        save_scores_to_db(epoch, np.array(step_loss).mean(), avg_val_loss)
