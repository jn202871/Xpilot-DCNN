...
train_loader = data.DataLoader(dataset, batch_size=16, shuffle=True, pin_memory=True, num_workers=4, drop_last=True)
...
model.load_state_dict(torch.load("./dcnn/models/retrain/retrained.pt", map_location=torch.device(device)))
...
torch.save(model.state_dict(), "./dcnn/models/retrain/retrained.pt")
